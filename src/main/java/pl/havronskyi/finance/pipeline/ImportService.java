...
public class ImportService {
     ...
     */
    public String startImport(Long accountId, String filename, byte[] content) {
        ImportJob job = jobRegistry.create();
        Thread.ofVirtual().name("import-" + job.getId()).start(() -> runImport(job, accountId, filename, content));
        return job.getId();
    }
     ...
     * not corrupting.
     */
    private void runImport(ImportJob job, Long accountId, String filename, byte[] content) {
        try {
            ...
            int byLlm = applyLlm(inserted, job);
    ...
    private int applyLlm(List<Txn> batch, ImportJob job) {
        ...
        List<CategorySuggestion> suggestions = llm.classify(pending, job::setProcessed, job::isCancelRequested);
     ...
     */
    @Transactional
    public int renormalizeMerchants() {
        int changed = 0;
        for (Txn t : txns.findAll()) {
     ...
     */
    @Transactional
    public int backfillMissingFxAmounts() {
        List<Txn> missing = txns.findByAmountPlnMinorIsNullAndCurrencyNot(props.baseCurrency());
        int fixed = 0;
        for (Txn t : missing) {
    ...
    public void clearTransactionData() {
        ...
        txns.deleteAllInBatch();
        raws.deleteAllInBatch();
